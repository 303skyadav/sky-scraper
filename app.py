...
raw_user = st.sidebar.text_input("👤 Username")
raw_pwd  = st.sidebar.text_input("🔒 Password", type="password")

# Strip leading/trailing spaces
user = raw_user.strip()
pwd  = raw_pwd.strip()

# Validate against secrets.toml
secrets = st.secrets.to_dict()
if (
    user != secrets.get("USERNAME")
    or pwd  != secrets.get("PASSWORD")
):
...
# (Your imports, st.set_page_config, etc., go below this)
...
# (scraping functions, sidebar filters, scraping loop, data cleaning,
#  charts, tables, export controls, etc.)


# ── Rest of your app follows below ────────────────────────────────────────
