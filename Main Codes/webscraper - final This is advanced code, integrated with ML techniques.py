...
st.title("🚀 SKY Scraper & Analyst V.3.0")
...
# — SIDEBAR FILTERS & “HOW TO” —————————————————————————————————————————
st.sidebar.header("How to Use")
st.sidebar.markdown("""
1. Enter a Myntra category URL and pages to scrape.  
2. Click **Auto Scrape**.  
3. Filter your data by:  
   - Company  
   - Discount range  
   - Ads only / Non-Ads  
4. Explore analytics & export your data.
""")
company_list  = st.sidebar.multiselect("Filter by Company", [])
...
if st.session_state.data:
    ...
    df["SellingVal"]    = pd.to_numeric(df["Selling Price"]
                                         .str.replace(r"[^\d]", "", regex=True),
                                         errors="coerce") \
                             .fillna(0).astype(int)
    df["MRPVal"]        = pd.to_numeric(df["MRP"]
                                         .str.replace(r"[^\d]", "", regex=True),
                                         errors="coerce") \
                             .fillna(df["SellingVal"]).astype(int)
    df["ComputedDiscount"] = ((df["MRPVal"] - df["SellingVal"]).clip(lower=0)
    ...
    # — TWO-UP ROW: Correlation Matrix & Segmentation —
    corr       = df[["Review Count","Rating","ComputedDiscount","SellingVal"]].corr()
    seg_counts = df["Company"].value_counts().head(3)  # placeholder; actual segments below

    # compute actual segments for bar chart
    labels = KMeans(n_clusters=3, random_state=0).fit_predict(
    ...
    # — TWO-UP ROW: Boxplot & Top 10 Companies —
    top10 = df["Company"].value_counts().head(10)

    c1, c2 = st.columns(2)
    with c1:
        fig, ax = plt.subplots(figsize=(6,4))
        data_to_plot = [df[df["Segment"]==s]["SellingVal"] for s in seg_counts.index]
    ...
    with c2:
        fig, ax = plt.subplots(figsize=(6,4))
        ax.bar(top10.index, top10.values, color="tab:green")
        ax.set_xticklabels(top10.index, rotation=45, ha="right")
        ax.set_title("Top 10 Companies by SKU Count")
    ...
    model     = LinearRegression().fit(df[["ComputedDiscount"]], df["Review Count"])
    test_disc = st.slider("What-If Discount (%)", 0, 100, 30)
    pred_rev  = model.predict([[test_disc]])[0]
    st.metric(f"Predicted Reviews @ {test_disc}% off", f"{int(pred_rev)}")

    st.markdown("---")

    # — TWO-UP ROW: Champions & Laggards —
    df["Score"] = df["Review Count"] * df["Rating"]
    champs = df.nlargest(3, "Score")[["Product","Company","Review Count","Rating"]]
    laggs  = df.sort_values(["ComputedDiscount","Rating"], ascending=[False,True]) \
    ...
    st.markdown(f"- Total SKUs: {kus_total}  \n- Avg Rating: {kus_avgrat:.2f}  \n- Avg Discount: {kus_avgdis:.1f}%")
    if kus_total:
        top10_k = kushal.sort_values("Review Count", ascending=False).head(10)
        st.table(
            top10_k[["Product","Review Count","Rating","ComputedDiscount","Selling Price","MRP"]]
            .rename(columns={"ComputedDiscount":"Discount (%)"})
        )

    st.markdown("---")

    # — ADVERTISED PRODUCTS ANALYSIS & CHARTS —————————————————————————————————
    adv = df[df["Ads"] == "Yes"]
    ...
    # — FULL DATAFRAME & DOWNLOAD —————————————————————————————————————
    st.dataframe(df.drop(columns=["SellingVal","MRPVal","Raw Discount","ComputedDiscount","Score"]), use_container_width=True)
