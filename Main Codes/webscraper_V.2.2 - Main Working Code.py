...
""",
    ...
    # ── Kushal vs. Top Peer Unified Comparison ─────────────────────────────────

    # 1) Filter Kushal SKUs
    kushal_df = filtered_df[filtered_df["Company"].str.contains("Kushal", case=False)].copy()

    # 2) Build one combined row per Kushal SKU
    rows = []
    for i, k in kushal_df.iterrows():
        subcat = k["Sub-category"]
        # pick the single top‐reviewed non-Kushal peer in same sub‐category
        peer = (
            filtered_df[
                (filtered_df["Sub-category"] == subcat) &
                (~filtered_df["Company"].str.contains("Kushal", case=False))
            ]
            .nlargest(1, "Review Count")
        )
        if peer.empty:
            continue
        p = peer.iloc[0]
        ...
        rows.append({
            "Sl No":              len(rows) + 1,
            "Kushal SKU":         k["Product"],
            "Kushal Page No.":    k["Page Number"],
            "Kushal Position":    k["Placement Index"],
            "Kushal Reviews":     k["Review Count"],
            "Kushal Rating":      k["Rating"],
            "Peer Company":       p["Company"],
            "Peer SKU":           p["Product"],
            "Peer Page No.":      p["Page Number"],
            "Peer Position":      p["Placement Index"],
            "Peer Reviews":       p["Review Count"],
            "Peer Rating":        p["Rating"],
        })

    # 3) Display as a single table
    if rows:
        comp_df = pd.DataFrame(rows)
        st.subheader("💎 Kushal vs. Top Peer Comparison")
        st.table(comp_df)
